...
该库是 Rime 输入法的一个 091 + 86版极点五笔的输入方案，支持多平台（Windows、macOS、Linux）

## 〇、说明

该库为个人定制版，新增091五笔码表的支持。词库与点儿词库保持同步更新。同时由于个人习惯做了以下变更：
1. `z键` 变更为 **重复上次输入记录**
2. **`** 设为 **拼音反查**
3. 二三候选是 <kbd>;</kbd> <kbd>'</kbd> 两个键；在输入时`Z键`可作为三选，方便左右互击；`Tab 键`可作为四选输入
